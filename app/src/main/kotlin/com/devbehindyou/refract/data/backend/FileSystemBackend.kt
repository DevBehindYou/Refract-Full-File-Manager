...
class FileSystemBackend @Inject constructor(
    ...
    override fun listChildren(id: FileNodeId): Flow<FileResult<List<FileNode>>> = flow {
        ...
        try {
            ...
            // A mid-enumeration failure after some chunks already succeeded has no clean
            // representation in this Flow<FileResult<List<FileNode>>> shape — the chunks
            // already emitted stand as-is. Documented limitation, not silently assumed away.
        }
    }
    ...
    override suspend fun exists(parent: FileNodeId, name: String): Boolean =
        File(fileFor(parent), name).exists()

    override suspend fun freeSpace(id: FileNodeId): Long = runCatching {
    ...
    private fun File.toFileNode(id: FileNodeId): FileNode = FileNode(
        id = id,
        name = name,
        displayName = name,
...
private class FileSystemOutputTarget(private val target: File) : OutputTarget {
    ...
    override fun sync() {
        // No separate fsync hook here: OutputTarget only exposes control after stream()
        // has already been handed out and (by the time toNode()/discard() are called)
        // presumably closed. A durable-write guarantee would need this backend to return
        // a stream wrapper whose close() calls FileDescriptor.sync() before closing —
        // not done here. Flagged in PHASE_3_NOTES.md rather than silently assumed durable.
    }
