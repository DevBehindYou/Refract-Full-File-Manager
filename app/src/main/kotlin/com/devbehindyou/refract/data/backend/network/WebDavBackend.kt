...
class WebDavBackend(
    ...
    private fun buildUrl(serverId: String, path: String): URL {
        ...
        val portPart = if ((protocol == "http" && server.port == 80) || (protocol == "https" && server.port == 443)) {
            ""
        } else {
            ":${server.port}"
        }
    ...
    private fun openConnection(url: URL, serverId: String, method: String): HttpURLConnection {
    ...
    override suspend fun getNode(id: FileNodeId): FileResult<FileNode> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val node = parsedNodes.firstOrNull()
                ?: NetworkNodeHelper.createNode(
                    id = id,
    ...
    override fun listChildren(id: FileNodeId): Flow<FileResult<List<FileNode>>> = flow {
        ...
        try {
            ...
            val children = nodes.filter { node ->
    ...
    override suspend fun openInput(id: FileNodeId): FileResult<InputStreamProvider> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (responseCode == 404) {
                ...
                return@withContext FileResult.Failure(FileError.StorageUnavailable("WebDAV response: $responseCode"))
            }
    ...
    private fun parseMultiStatusXml(xml: String, serverId: String): List<FileNode> {
        ...
        try {
            ...
            for (i in 0 until responses.length) {
                ...
                val size = if (lengthElements.length > 0) lengthElements.item(0).textContent.toLongOrNull() ?: 0L else 0L

                val dateElements = resp.getElementsByTagNameNS("*", "getlastmodified")
                val modified = if (dateElements.length > 0) {
                    runCatching { dateFormat.parse(dateElements.item(0).textContent)?.time }.getOrNull() ?: System.currentTimeMillis()
                } else {
