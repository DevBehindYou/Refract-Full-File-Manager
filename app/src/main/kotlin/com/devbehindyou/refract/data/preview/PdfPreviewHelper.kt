...
class PdfPreviewHelper(
    ...
    suspend fun getPageCount(id: FileNodeId): FileResult<Int> = withContext(Dispatchers.IO) {
        val tempFile = copyToCache(id) ?: return@withContext FileResult.Failure(FileError.IoFailure("Failed to cache PDF"))
        try {
    ...
    ): FileResult<Bitmap> = withContext(Dispatchers.IO) {
        val tempFile = copyToCache(id) ?: return@withContext FileResult.Failure(FileError.IoFailure("Failed to cache PDF"))
        try {
            ParcelFileDescriptor.open(tempFile, ParcelFileDescriptor.MODE_READ_ONLY).use { pfd ->
                PdfRenderer(pfd).use { renderer ->
    ...
    private suspend fun copyToCache(id: FileNodeId): File? = withContext(Dispatchers.IO) {
