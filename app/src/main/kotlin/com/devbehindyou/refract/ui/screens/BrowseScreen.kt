...
fun BrowseScreen(
    ...
    val secondaryViewModel = remember(initialFolderId.raw) {
        BrowseViewModel(
            initialFolderId = initialFolderId,
            getDirectoryListingUseCase = app.container.getDirectoryListingUseCase,
            getNodeUseCase = app.container.getNodeUseCase,
                ...
                BrowseTopBar(
                    ...
                    actions = BrowseTopBarActions(
                        ...
                        onToggleDualPaneMode = {
                            dualPaneMode = if (dualPaneMode == DualPaneMode.DUAL_BROWSE) {
                ...
                onHideNode = { node, mode ->
                    coroutineScope.launch {
                        when (mode) {
                            ...
                            HideMode.PRIVATE_STORAGE -> app.container.hiddenFilesRepository.moveToPrivateStorage(node)
                        }
                        viewModel.refresh()
                        nodeToHide = null
                    }
...
private fun handleBrowseKeyEvent(
    ...
    return when {
        ...
        event.key == Key.Escape -> {
            ...
            else if (uiState.isSearching) viewModel.toggleSearch(false)
            true
        }
        ...
        event.key == Key.Enter -> {
            if (uiState.selectedIds.size == 1) {
                val single = uiState.rawItems.firstOrNull { it.id in uiState.selectedIds }
                if (single != null) {
                    if (single.isDirectory) viewModel.navigateTo(single)
                    else if (onOpenFile != null) onOpenFile(single)
                    else onRequestPreview(single)
                }
            }
            true
        }
            ...
            actions = {
                ...
                if (isDualPane) {
                    IconButton(onClick = actions.onToggleDualPaneMode) {
                        Icon(
                            imageVector = if (dualPaneMode == DualPaneMode.DUAL_BROWSE) Icons.Filled.Preview else Icons.Filled.VerticalSplit,
                            contentDescription = if (dualPaneMode == DualPaneMode.DUAL_BROWSE) "Switch to Preview Pane" else "Switch to Dual Browse",
                        )
                    }
                }
                IconButton(
    ...
    if (isSelectionMode) {
        ...
        Surface(tonalElevation = 3.dp) {
            Row(
                ...
                Text(
                    text = "${clip.items.size} item(s) ready to " +
    ...
    Column(modifier = modifier.fillMaxSize()) {
            ...
            when (val status = op.status) {
                is OperationStatus.Running -> {
                    Column(
                        ...
                        val progressFraction = if (status.progress.itemsTotal > 0) {
        ...
        when {
            ...
            uiState.filteredItems.isEmpty() -> Box(
                ...
                Text(
                    text = if (uiState.searchQuery.isNotEmpty()) "No files match \"${uiState.searchQuery}\"" else "This folder is empty",
                ...
                LazyColumn(
                    ...
                    items(uiState.filteredItems, key = { it.id.raw }) { node ->
                        ...
                        val selectedNodes = if (isSelected) {
                        ...
                        val dragOrPeekMod = if (isMedia && !isSelectionMode) {
                        ...
                        val dropMod = if (node.isDirectory) {
