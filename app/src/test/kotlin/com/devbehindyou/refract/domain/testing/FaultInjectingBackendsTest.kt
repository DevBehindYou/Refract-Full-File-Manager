...
class FailAfterNBytesTest {
    ...
    @Test
    fun `writing fewer bytes than the threshold succeeds completely`() = runTest {
    ...
    @Test
    fun `writing past the threshold throws IOException`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `bytes written before the threshold are preserved in the underlying backend`() = runTest {
        val real = InMemoryBackend()
        val backend = FailAfterNBytes(thresholdBytes = 4, delegate = real)
    ...
    @Test
    fun `read operations pass through unaffected`() = runTest {
        val real = InMemoryBackend()
...
class DenyWriteTest {
    ...
    @Test
    fun `openOutput fails with AccessDenied`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `createDirectory fails with AccessDenied`() = runTest {
        val real = InMemoryBackend()
        val backend = DenyWrite(real)
    ...
    @Test
    fun `delete fails with AccessDenied and the node is not actually removed`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `rename fails with AccessDenied`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `moveWithin fails with AccessDenied`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `reads pass through unaffected`() = runTest {
        val real = InMemoryBackend()
...
class SlowBackendTest {
    ...
    @Test
    fun `getNode waits at least delayMillis of virtual time before returning`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `the delegate's result is still returned correctly after the delay`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `listChildren itself is not delayed, since it is not a suspend function`() = runTest {
...
class VanishingSourceTest {
    ...
    @Test
    fun `the target can be read exactly survivesReads times before vanishing`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `openInput on the target also vanishes after survivesReads`() = runTest {
        val real = InMemoryBackend()
    ...
    @Test
    fun `nodes other than the target are never affected`() = runTest {
        val real = InMemoryBackend()
